...
FOUNDATION_MARKER = "Generated by scripts/generate.py from palette/foundation.json and palette/aliases.json. Do not edit."
...
def generated_foundation_export(foundation: dict[str, Any], aliases: dict[str, Any]) -> str:
    document = {
        "generated": FOUNDATION_MARKER,
        "meta": foundation["meta"],
        "ramps": foundation["ramps"],
        "aliases": {
            style: {role: resolve_reference(foundation, reference) for role, reference in values.items()}
...
def generated_foundation_css(foundation: dict[str, Any], aliases: dict[str, Any]) -> str:
    lines = [f"/* {FOUNDATION_MARKER} */", ":root {"]
    for family, ramp in foundation["ramps"].items():
        css_family = family.replace("_", "-")
        for step, color in ramp.items():
            lines.append(f"  --nib-{css_family}-{step}: {color};")
    lines.append("}")
    for style in ("light", "dark"):
        selector = ':root, [data-nib-theme="light"]' if style == "light" else '[data-nib-theme="dark"]'
        lines.extend(["", f"{selector} {{"])
        for role, reference in aliases[style].items():
            variable = role.replace(".", "-").replace("_", "-")
            lines.append(f"  --nib-{variable}: {resolve_reference(foundation, reference)};")
        lines.append("}")
    return "\n".join(lines) + "\n"


def generated_showcase_data(palette: dict[str, Any], foundation: dict[str, Any]) -> str:
    document = {"palette": palette, "foundation": foundation["ramps"]}
    return f"// {FOUNDATION_MARKER}\nwindow.NIB_SHOWCASE = {json.dumps(document, separators=(',', ':'))};\n"


def generated_gimp_palette(foundation: dict[str, Any]) -> str:
    lines = ["GIMP Palette", "Name: Nib", f"# {FOUNDATION_MARKER}", "Columns: 10"]
    for family, ramp in foundation["ramps"].items():
        for step in ("400", "600"):
            color = ramp[step]
            channels = [int(color[index : index + 2], 16) for index in (1, 3, 5)]
            lines.append(f"{channels[0]:3d} {channels[1]:3d} {channels[2]:3d}\t{family.replace('_', ' ')} {step}")
    return "\n".join(lines) + "\n"


def generated_matplotlib_palette(foundation: dict[str, Any], aliases: dict[str, Any]) -> str:
    accent_names = ("blue_black", "moss", "teal", "burgundy", "rust", "violet", "amber", "sepia", "graphite")
    lines = [
        f'"""{FOUNDATION_MARKER}"""',
        "",
        "RAMPS = " + repr(foundation["ramps"]),
        "LIGHT = " + repr({role: resolve_reference(foundation, ref) for role, ref in aliases["light"].items()}),
        "DARK = " + repr({role: resolve_reference(foundation, ref) for role, ref in aliases["dark"].items()}),
        f"COLOR_CYCLE_LIGHT = {[foundation['ramps'][name]['600'] for name in accent_names]!r}",
        f"COLOR_CYCLE_DARK = {[foundation['ramps'][name]['400'] for name in accent_names]!r}",
        "",
        "def apply(style='light'):",
        "    import matplotlib as mpl",
        "",
        "    colors = LIGHT if style == 'light' else DARK",
        "    cycle = COLOR_CYCLE_LIGHT if style == 'light' else COLOR_CYCLE_DARK",
        "    mpl.rcParams.update({",
        "        'axes.facecolor': colors['background'],",
        "        'axes.edgecolor': colors['foreground.muted'],",
        "        'axes.labelcolor': colors['foreground.primary'],",
        "        'axes.prop_cycle': mpl.cycler(color=cycle),",
        "        'figure.facecolor': colors['background'],",
        "        'savefig.facecolor': colors['background'],",
        "        'text.color': colors['foreground.primary'],",
        "        'xtick.color': colors['foreground.secondary'],",
        "        'ytick.color': colors['foreground.secondary'],",
        "    })",
    ]
    return "\n".join(lines) + "\n"


def generated_r_palette(foundation: dict[str, Any]) -> str:
    names = ("blue_black", "moss", "teal", "burgundy", "rust", "violet", "amber", "sepia", "graphite")
    lines = [f"# {FOUNDATION_MARKER}"]
    for style, step in (("light", "600"), ("dark", "400")):
        values = ", ".join(f'{name} = "{foundation["ramps"][name][step]}"' for name in names)
        lines.append(f"nib_{style} <- c({values})")
    return "\n".join(lines) + "\n"
...
def render_files(palette: dict[str, Any]) -> dict[Path, str]:
    ...
    files: dict[Path, str] = {
        ...
        Path("css/nib-foundation.css"): generated_foundation_css(foundation, aliases),
        ...
        Path("dist/nib-foundation.json"): generated_foundation_export(foundation, aliases),
        Path("showcase/palette.js"): generated_showcase_data(palette, foundation),
