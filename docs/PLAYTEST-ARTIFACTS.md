...
- `metadata.json`: schema version, seed, generator and parameters, difficulty, the complete starting player/environment visual selection, initial movement-test-view state, fixed world-camera zoom, history identity, bindings, fixed-step rate, screenshot cadence, manual bookmark binding, solvability report, and every rejected seed.
...
- `events.jsonl`: frame-linked events for damage/death/fall, successful dash starts, terrain damage/destruction, directional tool strikes, successful or rejected rope placement, bomb explosions and outer-edge rocket jumps, burrower interactions, brittle triggers/collapse, relic collection, visual-pack and movement-test-view changes, manual playtest bookmarks, and summit completion.
