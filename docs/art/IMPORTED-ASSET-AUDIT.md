...
| `ruins.zip` | No bundled terms. It resembles [NicoPardo's STONE RUINS](https://nicopardo.itch.io/stone-ruins), whose page permits personal/commercial use, but the official listing is a differently named RAR with additional editable files | Restrained masonry fragments are a strong badlands ruin reference | Quarantined. Do not use until the download source or receipt confirms its identity. Even after confirmation, keep raw files out of a public repository unless redistribution permission is established. |
...
2. Confirm the provenance of `ruins.zip`; if it is the official Stone Ruins pack, its fragmented masonry is the closest tonal match.
