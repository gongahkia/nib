...
The optional visual selector loads the GandalfHardcore character and platformer PNGs directly from `local-only/` at runtime. It never copies them into tracked runtime content or publish output. `K` opens independent player and environment choices, including exact modular character layers and terrain-sheet bands; set `SUMMING_ART_ROOT` to the repository root when launching a build from elsewhere. This integration is for visual comparison and does not change the authoritative 24px tiles, materials, collision, generation, or one-tile player body.
