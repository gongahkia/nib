// Quireveil showcase ink stage: static, five texture samples, no time input.
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    float localEdge = smoothstep(0.014, 0.14, length(center.rgb - neighbors));
    float inkMask = smoothstep(0.045, 0.25, length(center.rgb - iBackgroundColor));
    float edgeMask = localEdge * inkMask;
    vec3 feathered = mix(center.rgb, neighbors, 0.085 * edgeMask);

    float density = qv_hash(floor(fragCoord * 0.5)) * 0.018;
