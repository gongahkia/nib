// Quireveil showcase paper stage: static, one texture sample, no time input.
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    float coarse = qv_hash(floor(fragCoord * 0.5)) - 0.5;
    float fine = qv_hash(floor(fragCoord)) - 0.5;
